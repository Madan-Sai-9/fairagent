...
**Next:** Phase 7's second deliverable — a quality trade-off check via a handful of full FedAvg runs, comparing final model accuracy under original phrased metadata vs. neutral templating, for the strategies that showed bias. A mitigation that removes bias but costs real selection quality is a finding in itself, not yet checked.
